...
namespace ade
{
	...
	void AdUnlitMaterialSystem::ReCreateMaterialDescPool(uint32_t materialCount)
	{
		...
		{
			LOG_E("DEscriptror Set max count is :{0} ,but request:{1}", NUM_MATERIAL_BATCH_MAX, newDescriptorSetCount);
			return;
		}
		...
		mMaterialDescriptorPool = std::make_shared<AdVKDescriptorPool>(device, newDescriptorSetCount*2, poolSizes);//because has params and resource desc. set

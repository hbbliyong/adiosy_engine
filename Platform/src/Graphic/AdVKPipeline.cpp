...
namespace ade
{
	AdVKPipelineLayout::AdVKPipelineLayout(AdVKDevice* device, const std::string& vertexShaderFile, const std::string& fragShaderFile, const ShaderLayout& shaderLayout /*= {}*/)
		:mDevice(device)
	{
		...
		CALL_VK(vkCreatePipelineLayout(device->GetHandle(), &pipelineLayoutInfo, nullptr, &mHandel));
	}
	...
	AdVKPipeline::AdVKPipeline(AdVKDevice* device, AdVKRenderPass* renderPass, AdVKPipelineLayout* pipelineLayout) 
	...
	AdVKPipeline* AdVKPipeline::SetVertexInputState(const std::vector<VkVertexInputBindingDescription>& vertexBindings, const std::vector<VkVertexInputAttributeDescription>& vertexAttrs)
	{
	...
	AdVKPipeline* AdVKPipeline::SetInputAssemblyState(VkPrimitiveTopology topology, VkBool32 primitiveRestartEnable /*= VK_FALSE*/)
	{
	...
	AdVKPipeline* AdVKPipeline::SetMultisampleState(VkSampleCountFlagBits samples, VkBool32 sampleShadingEnable, float minSampleShading /*= 0.f*/)
	{

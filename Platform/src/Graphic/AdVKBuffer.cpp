...
namespace ade
{
	AdVKBuffer::AdVKBuffer(AdVKDevice* device, VkBufferCreateFlags usage, size_t size, void* data, bool bHostVisible)
	...
	void AdVKBuffer::CreateBuffer(VkBufferUsageFlags usage, void* data)
	{
		...
		else
		{
			...
			void* mapping;
			vkMapMemory(mDevice->GetHandle(), stageMemory, 0, mSize, 0, &mapping);
			memcpy(mapping, data, mSize);
			vkUnmapMemory(mDevice->GetHandle(), stageMemory);

			CreateBufferInternal(mDevice, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT ,usage| VK_BUFFER_USAGE_TRANSFER_DST_BIT,
				 mSize, &mHandle, &mMemory);
